...
static int	join_all_threads(pthread_t *thr, pthread_t dt_thr, t_rules *rules)
{
	int	i;

	pthread_join(dt_thr, NULL);
	i = 0;
	while (i < rules->nb_philo)
		pthread_join(thr[i++], NULL);
	return (0);
...
static int	thread_alg(t_philo **philo, t_rules *rules)
{
	...
	{
		rules->stop = 1;
		join_all_threads(threads, death_thread, rules);
		return (free(threads), -1);
	}
	join_all_threads(threads, death_thread, rules);
	free(threads);
...
static void	ft_error_main(t_rules *rs, t_philo *pho, char *s)
{
	cleanup_mutexes(rs);
	free_philo(&pho);
	printf("%s\n", s);
...
int	main(int ac, char **av)
{
	...
	while (i < rules.nb_philo)
	{
		if (fill_philo(&philo, &rules, i) != 0)
			return (ft_error_main(&rules, philo, "Failed to initialize philo."), 0);
		i++;
	}
	if (thread_alg(&philo, &rules) != 0)
		return (ft_error_main(&rules, philo, "Failed thread_alg."), 0);
